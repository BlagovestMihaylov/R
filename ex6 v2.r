...
plot(model1.predictions, DF$y, xlim = c(-1, 20), ylim = c(-1, 20))
...
#       Информационни критерии (IC)
#       Информационните критерии (например BIC, AIC) са математически методи за оценка на 
#   това колко добре един модел описва данните, върху които е построен. В статистиката 
#   IC се използват за сравняване на различни възможни модели и определяне кой е 
#   най-подходящ за данните. Информационни критерии се състоят от 2 компоненти:
#       - брой на независимите (обясняващи) променливи, използвани за изграждане на 
#   модела.
#       - оценката на maximum likelihood на модела (колко добре моделът описва 
#   данните).
#
#       Най-подходящият модел според IC е този, който обяснява най-голямото количество 
#   вариации, използвайки възможно най-малко независими (обясняващи) променливи.


#       За целта на примера, ще използвам данни, които имат повече независимо променливи.
#   Данните са от множеството "mtcars". Зависимата променлива е разходът за гориво - 
#   "mpg"


#       Създаваме пълен модел, тоест всички променливи ще вземат участие
full_model <- lm(mpg ~ ., data = mtcars) 
summary(full_model)


#       Използваме Akaike Information Criteria (AIC) при избора на множество от 
#   обясняващи променливи. Целта е с минимален брой променливи да постигнем оптимални 
#   резултати.
#       Колкото по-малка е стойността на AIC, толкова по-добър модел е.
step_AIC_model <- stepAIC(full_model, direction = "both", trace = TRUE)
summary(step_AIC_model)
summary(full_model)


rm(list = c("step_AIC_model", "full_model"))
...
set.seed(6621)
XX <- runif(NN, 1, 5)
noise1 <- noise2 <- 3*rnorm(NN)
rho1 <- 0.8
rho2 <- 0.0
for(i in 2:NN) {  
  noise1[i] <- rho1*noise1[i-1] + sqrt(1 - rho1^2)*noise1[i] 
  noise2[i] <- rho2*noise2[i-1] + sqrt(1 - rho2^2)*noise2[i] 
}
#   Задаваме автокорелация равна на 0.8 и автокорелация 0.0


par(mfrow = c(2, 1))
  plot(noise1, type = "l", main = "Шум С автокорелация", ylab = "Стойност")
  plot(noise2, type = "l", main = "Шум БЕЗ автокорелация", ylab = "Стойност")
par(mfrow = c(1, 1))
...
model4 <- lm(y1 ~ XX)
model5 <- lm(y2 ~ XX)
#   С изключение на шума (остатъците), другите условия са равни
...
#       За model4, стойността на p-value e 0, следователно ИМАМЕ наличие на 
#   автокорелация. За model5, стойността на p-value e 0.5249 и следователно 
#   НЯМАМЕ наличие на автокорелация. 
...
LAG <- 30
par(mfrow = c(1, 2))
  x_r4 <- r4[1:(length(r4)-LAG)];   y_r4 <- r4[(1+LAG):length(r4)]
  plot(x_r4, y_r4, main = "Модел С автокорелация", xlab = paste("LAG", LAG), 
       ylab = "Сегашна стойност")
  abline(lm(y_r4 ~ x_r4), col = "forestgreen", lwd = 2)
  x_r5 <- r5[1:(length(r5)-LAG)];   y_r5 <- r5[(1+LAG):length(r5)]
  plot(x_r5, y_r5, main = "Модел БЕЗ автокорелация", xlab = paste("LAG", LAG), 
       ylab = "Сегашна стойност")
  abline(lm(y_r5 ~ x_r5), col = "forestgreen", lwd = 2)
par(mfrow = c(1, 1))
...
autocorrelation <- TSA::acf(r4, lag.max = 40)
...
cor(r4[1:(length(r4)-LAG)], r4[(1+LAG):length(r4)])



rm(list = c("i", "model4", "model5", "noise1", "noise2", "rho1", "rho2", "XX", "y1", "y2",
            "x_r4", "x_r5", "y_r4", "y_r5", "r4", "r5"))
